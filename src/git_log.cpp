...
GitLogFunctionData::GitLogFunctionData(const string &ref) : repo_path(""), resolved_repo_path(""), ref(ref) {
}
...
unique_ptr<FunctionData> GitLogBind(ClientContext &context, TableFunctionBindInput &input,
	...
	names = {"repo_path",   "commit_hash", "author_name", "author_email", "committer_name", "committer_email",
...
unique_ptr<LocalTableFunctionState> GitLogLocalInit(ExecutionContext &context, TableFunctionInitInput &input,
...
void GitLogFunction(ClientContext &context, TableFunctionInput &data_p, DataChunk &output) {
	...
	if (!local_state.initialized) {
		...
		if (error != 0) {
			...
			throw IOException("Failed to open git repository '%s': %s", bind_data.repo_path,
...
static OperatorResultType GitLogEachFunction(ExecutionContext &context, TableFunctionInput &data_p, DataChunk &input,
                                             DataChunk &output) {
	auto &bind_data = data_p.bind_data->Cast<GitLogFunctionData>();
	...
	while (true) {
		if (!state.initialized_row) {
			...
			if (state.cached_repo_path != resolved_repo_path) {
				...
				if (error != 0) {
					const git_error *e = git_error_last();
					throw IOException("Failed to open repository '%s': %s", resolved_repo_path,
					                  e ? e->message : "Unknown error");
				}
			...
			ProcessLogCommitForInOut(state.cached_repo, resolved_repo_path, final_ref, state.current_rows,
			                         resolved_file_path);
