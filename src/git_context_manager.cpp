...
GitContextManager::GitContext::GitContext(GitContext &&other) noexcept
    : resolved_object(other.resolved_object),
      repo_path(std::move(other.repo_path)), file_path(std::move(other.file_path)),
      final_ref(std::move(other.final_ref)) {
...
git_object *GitContextManager::ValidateAndResolveReference(const string &repo_path, const string &ref) {
    ...
    if (error != 0) {
        ...
        throw IOException("GitContextManager: Failed to open repository '%s': %s",
    ...
    if (error != 0) {
        ...
        if (error_msg.find("unable to parse") != string::npos ||
            error_msg.find("invalid characters") != string::npos ||
            error_msg.find("not found") != string::npos) {
            throw IOException("unable to parse OID");
        } else {
            throw IOException("GitContextManager: Failed to resolve ref '%s' in repository '%s': %s",
                            ref, repo_path, error_msg);
        }
    }
    return obj;
}
